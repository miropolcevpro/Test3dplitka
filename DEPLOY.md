P10 workspace photo area cleanup
- Верхние guided-подсказки и stepper убраны из постоянного layout-контура, вместо них оставлен компактный шаговый индикатор в шапке.
- Рабочая зона фото увеличена по высоте, toolbar уплотнён, контурные action-кнопки сохранены в компактном виде.
- После деплоя обязателен hard refresh, чтобы подтянуть новый CSS/JS.
...
## P09-HF2 forms bottom visibility hotfix
- Исправлена причина, из-за которой нижняя панель форм могла полностью уходить из видимой области на desktop/iframe-экранах.
- Убрана жёсткая высокая высота `canvasWrap`, из-за которой `bottomMenu` оказывался за пределами `canvasPane` и клиппировался `overflow:hidden`.
- `canvasWrap` сделан адаптивным по высоте, а `bottomMenu` закреплён внизу flex-колонки через `margin-top:auto`.
- После деплоя выполните hard refresh iframe/страницы, чтобы подтянулся новый CSS.
