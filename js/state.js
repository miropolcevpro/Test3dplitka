...
window.PhotoPaveState=(function(){
  ...
  const state={
    build:{version:"mvp-iter1.13-contour-perspective-no-plane-fix",ts:new Date().toISOString()},
  ...
  const snapshot=()=>JSON.stringify({ui:{activeStep:state.ui.activeStep,mode:state.ui.mode,activeZoneId:state.ui.activeZoneId,activeCutoutId:state.ui.activeCutoutId},zones:state.zones,catalog:{activeShapeId:state.catalog.activeShapeId}});
  const restore=(json)=>{const s=JSON.parse(json);state.ui.activeStep=s.ui.activeStep;state.ui.mode=s.ui.mode;state.ui.activeZoneId=s.ui.activeZoneId;state.ui.activeCutoutId=s.ui.activeCutoutId;state.zones=s.zones;state.catalog.activeShapeId=s.catalog.activeShapeId;};
