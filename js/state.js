...
window.PhotoPaveState=(function(){
  ...
  const state={
    build:{version:"mvp-iter1",ts:new Date().toISOString()},
    api:{gatewayBase:DEFAULT_GATEWAY,apiBase:DEFAULT_GATEWAY,config:null},
  ...
  const makeZone=()=>({id:uid("zone"),name:"Зона "+(state.zones.length+1),enabled:true,contour:[],cutouts:[],material:{shapeId:state.catalog.activeShapeId||null,textureId:null,textureUrl:null,params:{scale:1.0,rotation:0,opacity:0.85,blendMode:"multiply"}}});
  const makeCutout=(n)=>({id:uid("cut"),name:n?("Вырез "+n):"Вырез",polygon:[]});
