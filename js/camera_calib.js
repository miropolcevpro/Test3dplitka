...
function _softClamp01(v){
  ...
  // Quick "2 lines" calibration (test): user provides one guide line for depth (A1)
  // and one guide line for cross direction (B1). We assume camera roll is small and
  // solve for a horizontal horizon line y = yHpx via 1D search. Vanishing points are
  // intersections of each guide with the horizon.
  function computeFromGuides(guides, imgW, imgH){
    const W = Math.max(1, imgW||1);
    const H = Math.max(1, imgH||1);
    const c = {x: W*0.5, y: H*0.5};

    const LA = lineFromPts(guides && guides.A1 && guides.A1.p1, guides && guides.A1 && guides.A1.p2);
    const LB = lineFromPts(guides && guides.B1 && guides.B1.p1, guides && guides.B1 && guides.B1.p2);
    if(!LA || !LB) return {ok:false, reason:"missing_guides"};

    // Guide lines should not be horizontal (can't intersect a horizontal horizon robustly).
    if(Math.abs(LA.a) < 1e-6) return {ok:false, reason:"A:degenerate"};
    if(Math.abs(LB.a) < 1e-6) return {ok:false, reason:"B:degenerate"};

    const fMin = 0.25 * Math.max(W, H);
    const fMax = 6.0 * Math.max(W, H);
    const distMax = 60 * Math.max(W, H);

    function intersectWithHorizon(L, yH){
      // L: a*x + b*y + c = 0 => x = -(b*y + c)/a
      const x = - (L.b * yH + L.c) / L.a;
      if(!isFinite(x)) return null;
      return {x, y:yH};
    }

    // Penalty-based search for horizon y (pixels)
    let best = {pen:1e30, yH:null, VA:null, VB:null, f:null};
    const yMin = -2.0 * H;
    const yMax =  2.0 * H;
    const steps = 220;
    ...
    for(let i=0;i<=steps;i++){
      const t = i/steps;
      const yH = yMin + (yMax - yMin) * t;

      const VA = intersectWithHorizon(LA, yH);
      const VB = intersectWithHorizon(LB, yH);
      if(!VA || !VB) continue;

      const distApx = Math.hypot(VA.x - c.x, VA.y - c.y);
      const distBpx = Math.hypot(VB.x - c.x, VB.y - c.y);
      if(distApx > distMax || distBpx > distMax) continue;
      ...
      // Penalties: focal outside range, and horizon too extreme.
      let pen = 0.0;
      if(f < fMin) pen += (fMin - f) * (fMin - f);
      if(f > fMax) pen += (f - fMax) * (f - fMax);

      // Prefer a horizon not too far outside the image (soft)
      const ySoftMin = -0.6*H;
      const ySoftMax =  1.6*H;
      if(yH < ySoftMin) pen += (ySoftMin - yH) * (ySoftMin - yH) * 0.002;
      if(yH > ySoftMax) pen += (yH - ySoftMax) * (yH - ySoftMax) * 0.002;

      // Prefer not extremely large f (stability)
      pen += ((f / Math.max(1,Math.max(W,H))) - 1.2) ** 2 * 30.0;
    ...
    if(!best.VA || !best.VB || !isFinite(best.f)) return {ok:false, reason:"no_solution"};

    const VA = best.VA;
    const VB = best.VB;
    const f = best.f;

    const horizonY = _clamp(best.yH / H, -2, 2);
    const vanishANorm = {x: VA.x/W, y: VA.y/H};
    const vanishBNorm = {x: VB.x/W, y: VB.y/H};
    const ctrl = deriveControlsFromVanish(vanishANorm);
    ...
    // Approximate camera basis same way as computeFromLines (uses vanishing points)
    const r1raw = {x:(VA.x - c.x)/f, y:(VA.y - c.y)/f, z:1};
    const r2raw = {x:(VB.x - c.x)/f, y:(VB.y - c.y)/f, z:1};
    // Normalize to unit
    const n1 = 1/Math.max(EPS, Math.hypot(r1raw.x,r1raw.y,r1raw.z));
    const n2 = 1/Math.max(EPS, Math.hypot(r2raw.x,r2raw.y,r2raw.z));
    const r1 = {x:r1raw.x*n1, y:r1raw.y*n1, z:r1raw.z*n1};
    const v2 = {x:r2raw.x*n2, y:r2raw.y*n2, z:r2raw.z*n2};
    // Gram-Schmidt for stability
    const dot12 = (r1.x*v2.x + r1.y*v2.y + r1.z*v2.z);
    const v2o = {x:v2.x - dot12*r1.x, y:v2.y - dot12*r1.y, z:v2.z - dot12*r1.z};
    const n2o = 1/Math.max(EPS, Math.hypot(v2o.x,v2o.y,v2o.z));
    const r2 = {x:v2o.x*n2o, y:v2o.y*n2o, z:v2o.z*n2o};
    const r3 = {x:(r1.y*r2.z - r1.z*r2.y), y:(r1.z*r2.x - r1.x*r2.z), z:(r1.x*r2.y - r1.y*r2.x)};
    const R = {r1, r2, r3};

    // Confidence: 1 for low penalty, 0 for high.
    const conf = _clamp(1.0 - (best.pen / 800.0), 0, 1);
