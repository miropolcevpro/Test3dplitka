...
try{
  if(Hm){
    ...
    // Perspective slider -> focal scaling (physically: higher "perspective" => wider FOV => smaller focal)
    const kPersp = 0.80;
    const perspT = (pVal - 0.5) * 2.0; // [-1..1]
    const fCur = clamp(fGuess * Math.exp(-perspT * kPersp), 0.35*maxDim, 4.0*maxDim);

    // Reference camera for anchor-scale lock: default UX baseline (h=0, p=0.75)
    const pRef = 0.75;
